...
export const ALLOW_COMMON_FILE_TYPES_FOR_IMG = ['image/jpg', 'image/jpeg', 'image/png']
// export const ALLOW_COMMON_FILE_TYPES_FOR_ATTACHMENTS = [...ALLOW_COMMON_FILE_TYPES_FOR_IMG, ]
export const singleFileValidator = (file) => {
  ...
  if (!ALLOW_COMMON_FILE_TYPES_FOR_IMG.includes(file.type)) {
    return 'File type is invalid. Only accept jpg, jpeg and png'
  }
  return null
}

export const singleFileValidatorForAttachment = (file) => {
  if (!file || !file.name || !file.size || !file.type) {
    return 'File cannot be blank.'
  }
  if (file.size > LIMIT_COMMON_FILE_SIZE) {
